...
from apps.honeypot.models import LoginAttempt


class LoginAttemptAdmin(BaseAdmin):
    list_display = (
        "username", "get_ip_address", "get_session_key", "timestamp",
        "get_path"
    )
    list_filter = ("timestamp",)
    readonly_fields = (
        "path", "username", "ip_address", "session_key", "user_agent", "id",
    ...
    def get_session_key(self, instance):
        return format_html(
            "<a href=\"?session_key={sk}\">{sk}</a>",
            sk=instance.session_key
        )
