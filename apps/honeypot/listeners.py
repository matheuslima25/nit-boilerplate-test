...
from apps.honeypot.signals import honeypot
...
def notify_admins(instance, request, **kwargs):
    ...
    subject = render_to_string("admin_templates/email_subject.txt", context).strip()
    message = render_to_string("admin_templates/email_message.txt", context).strip()
